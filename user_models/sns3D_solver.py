...
class _DamageTracker:
    def __init__(self):
        ...
        self.achieved_lambda: NDArray[np.float64] | None = None
...
def simulate(params_si: dict, tspan: float, context: dict):
    ...
    if damage_Y <= 1.0:
        raise ValueError("SNS3D requires damage_Y > 1.")
    ...
    def rhs(_t: float, x: NDArray[np.float64]) -> NDArray[np.float64]:
        ...
        if _t < tracker.t_rmax_star or tracker.damage_mask is None:
            xi2 = np.ones(MT, dtype=float)
        else:
            xi2 = np.ones(MT, dtype=float)
            xi2[tracker.damage_mask] = 0.0
        ...
        s_neq2 = np.trapz(xi2 * 3.0 / term_r * s2, term_r)
        s2_dot = (-4.0 / Re2 * lambda_dot_out / lambda_out - s2) / De2
        s2_dot_term1 = xi2 * 3.0 * U * R**2 / (term_r**4) * s2
        s2_dot_term2 = xi2 * 3.0 / term_r * s2_dot
        s_neq2_dot = -np.trapz(s2_dot_term1, term_r) + np.trapz(s2_dot_term2, term_r) - xi2[0] * 3.0 / R * s2[0] * U
    ...
    if len(sol_pre.t_events[0]) > 0:
        ...
        lambda_rmax = (
            1.0 + (1.0 / r0_star_list**3) * (lambda_w_rmax**3 - 1.0)
        ) ** (1.0 / 3.0)
        tracker.t_rmax_star = t_rmax_star
        tracker.damage_mask = lambda_rmax >= damage_Y
        tracker.achieved_lambda = lambda_rmax.copy()

        if (
            damage_Y > 1.0
            and float(lambda_rmax[-1]) < damage_Y < float(lambda_rmax[0])
        ):
            r0_y = ((lambda_w_rmax**3 - 1.0) / (damage_Y**3 - 1.0)) ** (1.0 / 3.0)
            idx_ins = int(np.searchsorted(r0_star_list, r0_y))
            r0_star_list = np.insert(r0_star_list, idx_ins, r0_y)
            MT = MT + 1
            s1_0 = np.insert(s1_0, idx_ins, 0.0)
            s2_0 = np.insert(s2_0, idx_ins, 0.0)
            y0 = np.concatenate(([R0_star, U0_star, P0_star, Seq0], theta0, k0, s1_0, s2_0))
            idx_s2 = idx_s1 + MT
